  ...
  Widget _buildSensorCards({
    ...
    final cards = [
      ...
      _SensorCard(
        ...
        muted: muted,
        label: 'Temperature',
        displayValue: tempC != null ? tempVal : '—',
          ...
          Flexible(
            child: Column(
              ...
              children: [
                Text(
                  'ACTIVE PROFILE',
                  style: _ts(muted, size: 10, spacing: 0.8),
                ),
                const SizedBox(height: 3),
                Text(
                  cropProfile?['name'] ?? 'No profile',
                  style: _ts(text, size: 16, weight: FontWeight.bold),
      ...
      Row(
        ...
        children: [
          ...
          Flexible(
            child: Text(
              value,
              style: _ts(text, size: 11, weight: FontWeight.bold),
              overflow: TextOverflow.ellipsis,
            ),
          ),
        ],
                      ...
                      LineChartData(
                        ...
                        lineBarsData: [
                          ...
                          LineChartBarData(
                            ...
                            belowBarData: BarAreaData(
                              show: true,
                              color: cTemp.withValues(alpha: 0.08),
                            ),
                          ),
                          // Humidity
                          LineChartBarData(
                            ...
                            belowBarData: BarAreaData(
                              show: true,
                              color: cHumid.withValues(alpha: 0.08),
                            ),
                          ),
                        ],
                      ),
...
class _DashboardPumpCardState extends State<_DashboardPumpCard> {
  ...
  void _startTimers() {
    ...
    _safetyTimer = Timer(_kDashboardSafetyLimit, () async {
      ...
      if (id != null) await _sendCommand(id, 'off');
      if (mounted) {
  ...
  Future<bool> _sendCommand(String deviceId, String command) async {
    try {
      bool mqttSuccess = context.read<MqttService>().sendPumpCommand(
        deviceId,
        command,
      );

      // Always insert to Supabase as fallback (ESP32 polls this table)
      await Supabase.instance.client.from('device_commands').insert({
        'device_id': deviceId,
        'command': command,
      });

      // If MQTT failed, show warning but still return true (Supabase fallback exists)
      if (!mqttSuccess) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('MQTT unavailable — using cloud fallback'),
              backgroundColor: Color(0xFFF59E0B),
              behavior: SnackBarBehavior.floating,
            ),
          );
  ...
  Future<void> _toggle() async {
    ...
    if (!_isRunning) {
      ...
      final sent = await _sendCommand(id, 'on');
      if (!mounted) return;
