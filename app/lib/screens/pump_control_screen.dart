  ...
  Future<void> _togglePump(String deviceId, String command) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Pump \$command sent successfully.')));
      } else {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to send command. Server returned \${response.statusCode}')));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error connecting to backend: \$e')));
    } finally {
    ...
    return Scaffold(
      body: Center(
        child: Column(
          ...
          children: [
            Text('Manual Pump Control', style: Theme.of(context).textTheme.headlineMedium),
            ...
            if (deviceId == null) 
              ...
              ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: Colors.green, padding: const EdgeInsets.all(24)),
              ),
